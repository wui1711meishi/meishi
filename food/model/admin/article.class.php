 ...
 * Time: 17:21
 */
class article extends adminPar{
    function init(){
    ...
    function fun($db,&$str,$id=1){
        ...
        foreach($arr as $item){
            $str.="<tr class='' align='center'>
                ...
                <td>{$item['cid']}</td>
                <td>
                    <a href='index.php?m=admin&f=article&a=newarticle&id={$item['id']}&cid={$item['cid']}' class='btn btn-success update'>编辑</a>
                    <a href='index.php?m=admin&f=article&a=delarticle&id={$item['id']}'  class='btn btn-danger del' >删除</a>
                </td>
            </tr>";
    ...
    function show(){
        ...
        foreach($arr as $item){
            $str.="<tr class='' align='center'>
                ...
                <td>{$item['look']}</td>
                <td>
                    <a href='index.php?m=admin&f=article&a=newarticle&id={$item['id']}' class='btn btn-success update'>编辑</a>
                    <a href='index.php?m=admin&f=article&a=checkdelarticle&id={$item['id']}'  class='btn btn-danger del' >删除</a>
                </td>
            </tr>";
        }
    ...
    function addarticle(){
        ...
        $str='';
        $this->smarty->assign('str',$str);
        $this->smarty->display('admin/addarticle.html');
        $db->close();
    }
    //确认添加内容
...
//        $pfdj=$_POST['pfdj'];
        ...
        if($row==1){
            $this->jump('添加完成！','index.php?m=admin&f=article&a=show');
        }else{
            $this->jump('添加失败！','index.php?m=admin&f=article&a=show');
        }
        $db->close();
    }
    //递归得出所有固定分类目录
...
//            $str.="<option value='{$item['id']}'>$pad{$item['name']}</option>";
    ...
    function newarticle(){
        ...
        $this->smarty->display('admin/newarticle.html');
        $db->close();
    }
    //确认修改内容
...
//        $pfdj=$_POST['pfdj'];
        ...
        if($row==1){
            $this->jump('修改完成！','index.php?m=admin&f=article&a=show');
        }else{
            $this->jump('修改失败！','index.php?m=admin&f=article&a=show');
        }
        $db->close();
    }
    //确认删除内容
    function checkdelarticle(){
        $id=$_REQUEST['id'];
        $db=new db();
        $db->table='article';
        $row=$db->del("id='$id'");
        if($row==1){
            $this->jump('删除完成！','index.php?m=admin&f=article&a=show');
        }else{
            $this->jump('删除失败！','index.php?m=admin&f=article&a=show');
        }
        $db->close();
    }
    //图片上传
